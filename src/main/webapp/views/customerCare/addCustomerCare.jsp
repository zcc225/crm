	...
	<form class="layui-form" lay-filter="followup-form" style="width: 96%">
        ...
        <div class="layui-form-item" id="customer" style="display: none">
            ...
            <div class="layui-input-inline" style="width: 320px;">
                <select class="layui-input" name="linkmanId" lay-verify="customerSelect" >
		...
		<div class="layui-form-item">
	        ...
	        <div class="layui-input-inline">
	            <textarea class="layer-input" name="detail" lay-verify="required" style="height: 100px;width: 320px"></textarea>
	        </div>
        </div>		
...
layui.use(['form','upload','laydate'],function(){
	...
	var url = '${pageContext.request.contextPath}/customerCare/addCustomerCare';
	...
	//if(parm.type == 'add'){
		...
		if(parm.customerId == null || parm.customerId == ''){
			...
			$('#customer').show();
			//加载客户列表
			$.post('${pageContext.request.contextPath}/linkman/list',{'type':'all'},function(data){
	            var customers = data.data;
	            var str = '<option value="">--请选择客户--</option>'
	            for(var i=0;i<customers.length;i++){
	            	str += '<option value="' + customers[i].id + '">' + customers[i].name + '</option>';
	            }
	            $('select[name=linkmanId]').html(str);
	            form.render('select');
	        });
			
		}else{
			 
			 $('select[name=linkmanId]').html('<option value="' + parm.customerId + '">--请选择客户--</option>')
			 form.render('select');
